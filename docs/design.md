...
Dark mode mirrors every role with a `-dark` token: warm near-black `background-dark` (#131312) and off-white `text-primary-dark` (#E8E8E2) — never pure black/white, which glares during night reading — and `highlight-dark` (#5A5122), the same yellow compressed to an ember that keeps 7:1 text contrast. Theme is a per-device preference (localStorage, PRD §2.1). Light mode additionally exposes two runtime comfort variables from the PRD's sliders, both implemented as fixed, pointer-events-none full-canvas overlays: `--reading-dim` (0–0.15 opacity of a black overlay; the Settings slider presents it as brightness steps **1–20**, matching the reference device's scale, mapped linearly so step 20 is undimmed) and `--reading-warmth` (0–0.18 opacity of an amber overlay, #E0B464). Both live outside the token palette, apply to the whole canvas, and are never active in dark mode. Brightness must never be a `filter` on the app root: any filter (or transform) on `<body>` silently becomes the containing block for every `position:fixed` element, anchoring the bottom nav, toasts, and dialog scrims to the end of the document instead of the viewport — the overlay achieves the same dimming without touching positioning. The active theme must also be declared to the platform: set `color-scheme: light` / `color-scheme: dark` on the root (plus `<meta name="color-scheme" content="light dark">`) so native surfaces — the mobile on-screen keyboard, scrollbars, form controls — render in the matching light/dark variant, and set the PWA `theme-color` meta per theme so installed-app chrome matches. Exact keyboard colors cannot be themed (the keyboard belongs to the OS and the user's keyboard app); light/dark agreement is the achievable and expected behavior.
...
**Filters & chips.** The Collection filters live in a **filters-panel** — a pop-out `surface` card with a `border-strong` hairline (the same contained-surface family as the select-bar) — opened from a fixed-size funnel `icon-btn` in the toolbar on every viewport. The icon never changes size; when any filter is active a **filter-badge** — a small filled-ink circular bubble on the icon's corner — carries the active count, so a filter hidden behind the closed panel can never silently make cards "disappear" (the book footer's "n of m cards shown" line backs it up). Inside, **one labeled row per filter dimension**, captions aligned in a column: **Reviewed: All / Yes / No**, **Exported: All / Yes / No**, and **Notes: All / Yes / No** as compact segmented toggles (the `mode-toggle` pill at `mode-toggle-sm` scale — a yes/no question reads cleaner as a three-way switch than as on/off chips); the **Notes** row filters on whether a card carries any reader annotation — a card-level note or a highlight (with or without a note of its own) — so a card the reader only highlighted, never writing a word, still surfaces here; **Starred: All / ★ Starred** as the same segmented pattern (a filter dimension like the others, not a stray chip); and **Tags** as `chip-filter` "bubble filter" pills, **multi-select**: any number active at once, a card matching *any* selected tag shows (OR within the row; rows AND together, along with the saved-card text search), with an "All" chip that clears the set — the active chip inverts to filled ink (`chip-filter-active`), exactly like the reference's selected filter. A panel footer offers **Clear all** (shown only when something is active) and **Done**. The exported state is stamped on a card whenever it is included in a download (`lastExportedAt`), the same way revealing an answer stamps `lastReviewedAt`. `chip-tag` is the smaller bordered pill showing a card's own tags on the card face.

**Collection.** The page (titled **My Collection**) reads top-down: the title, the saved-cards search field directly beneath it (full `input-search` anatomy — it filters the grid live and sits with the content it filters, never in the top bar), a single toolbar row with the **Overview / Flashcards** display toggle (`mode-toggle`) on the left and, pinned to the column's right edge, the funnel filter icon (with its count badge) and one **Select** entry button (`button-outline`) — every control fixed-size, so the row never reflows — then the filters-panel when open, then the grid. The grid pages at 12 cards: a centered pager of two `button-text` controls (← PREVIOUS / NEXT →) flanking a `caption` page line (PAGE 1 OF 2) sits between the grid and the book footer, and any filter change snaps back to page 1. Every card carries a corner **star** `icon-btn` (☆ outline / ★ filled) the reader taps to mark for recall — `@click.stop` so it never triggers the card's own tap.

`card-flashcard` (Overview mode) is a `surface` panel with hairline border and `lg` padding: Mulish for the term and metadata, Literata for the summary, tag chips below. When the card carries an annotation, the card-level note renders **inline** just beneath the summary — italic Literata behind a 2px `border-strong` left rule, clamped to two lines, so the reader's own words are set visibly apart from the encyclopedic summary above — with a `caption` tally of its highlights below it, shown as ✎ n highlight notes (the count is every highlight, noted or not, so a card the reader only highlighted still shows a line here); a card with neither a note nor a highlight shows nothing extra. Hover deepens the border to `border-strong`, adds `shadow-sm`, and reveals edit/delete controls (always visible on touch). `card-flip` (Flashcards mode) is the same panel that **flips whole**: the card element is a transparent 3D stage and the visible surface (background, hairline, radius) lives on its two **faces**, so a 250ms 3D `rotateY` turns the entire card — not just its contents — between a front face (the term alone, set in `title`, centered) and a back face (the answer in Literata that scrolls from the top, plus a hairline-topped foot). A card with any annotation marks its front face with a small ✎ `note-ind` (a quiet Mulish promise that there is something saved to read), and its back foot splits into two `button-text` cells — an uppercase-caption **VIEW FULL ARTICLE** beside **✎ NOTES (n)**; a card with no annotations keeps the single full-width article strip. The article strip lives *on the back face only*, so it exists exactly when the answer does — there is no way to open the article without first revealing it. The **✎ NOTES** button turns the back into a **notes face**: an overlay (not a third 3D face — the flip stays a two-face `rotateY`) that reads the reader's annotations formatted to keep *whose words* clear — the card note first in italic Literata behind its left rule, then each highlighted passage as an upright-Literata quote carrying its `highlight` yellow `<mark>` with the reader's `↳` note in italic beneath it (a bare highlight shows the quote alone), `caption` labels heading each section, and a **← BACK TO DEFINITION** foot as the only way out. The notes face is transient UI, never persisted: a flip, an advance, a shuffle, or a mode switch always lands back on the term or definition, never on notes. Tap flips both ways; the article strip opens the frozen detail record (never live Wikipedia). The `session-card` carries the identical ✎ indicator, split foot, and notes face.

**Selecting cards.** One **Select** button opens a neutral selection mode. Rather than morph the browse toolbar, a dedicated **select-bar** appears as its own row — a `surface` banner with a `border-strong` hairline (the "you are picking cards" state) — reading `N selected` on the left, three management `button-text` verbs **Select all · Clear · Cancel**, and then every action verb at once with a deliberate hierarchy: **Review (n)** as the `button-primary` (the common case), **Export (n)** as `button-outline`, and **Delete (n)** as a quiet `button-text` — the destructive verb gets the *lowest* visual weight, and its safety is a confirm step, never red. All three disable until at least one card is picked. Select all operates on the currently *filtered* set, so filter-to-a-topic → Select all → Review is two taps. Opening selection folds the filters-panel away (one contextual surface at a time; the funnel icon stays available for a deliberate re-open), and on narrow viewports the select-bar turns `position: sticky` just below the top bar so the verbs stay visible while the grid scrolls. This is the standard select-then-act pattern (as in a mail or photos app): with three verbs, one neutral door scales where intent-scoped doors would crowd the toolbar and hand Delete top-level prominence.

**Review session (focused mode).** Launching Review opens a distinct full-column study view (its own route). A single `session-card` sits centered — the same whole-card flip anatomy as `card-flip` but sized large (`min(52vh, 420px)`), with the full (unclamped) answer on the back and a corner **star** `icon-btn` (outside the flip area, so it never rotates) for marking a hard term mid-session — starred cards resurface later through the Starred filter. Every way of moving through the deck plays the same 240ms tinder-style **fling** (the current card flies off with a slight rotation; the next rises in face-down): on touch the card literally follows the finger — drag left for next, right for previous, snap-back under an 80px threshold, vertical scrolling untouched — while the ← / → arrow pair (`icon-btn`, prev disabled on the first card) and the keyboard arrows drive the identical animation on desktop. Once the answer is revealed, two `verdict-btn` controls — **✗ Wrong** and **✓ Right** — record how the reader did and ride the same fling to the next card; they stay disabled until the card is flipped, so a face is never graded before it is seen, and the chosen verdict inverts to filled (`verdict-btn-marked`). Advancing with **→** (or a swipe) instead leaves no verdict, and that counts as a **skip**. Below the card, `session-controls`: the arrow pair flanking a `caption` position line (`3 / 12`); above it, a `caption` session count and a **shuffle** `icon-btn`. The whole flow is keyboard-native (arrows move, space flips), and revealing an answer stamps the card reviewed. A detour into a card's full detail via the article strip and back resumes the session in place. Finishing the last card lands on a results `state-box`: a pass/fail line (the reader passes when correct answers outnumber wrong-plus-skipped), the score line (`You got 7/10 correct, and skipped 0 cards`), and two ways forward — **Restart** the whole deck, or **Revise** just the wrong-and-skipped cards — beside a quiet return to the collection. Monochrome throughout: right and wrong are told by glyph, label, and the filled/outline state, never by colour.

**Saving.** `bookmark-save` is the single save control on the results and detail pages — a bookmark icon sized to the `title` beside it (outline = unsaved, filled = saved). It replaces a labeled "Save" button: in this monochrome system a filled-vs-outline icon carries the saved state at a glance. Saving is immediate and wordless — one click saves the card (a `toast` confirms) with no dialog interrupting the read, and the page smooth-scrolls to the notes editor below (instant under reduced motion) so the next step is never stranded under the fold; clicking the filled bookmark again unsaves it (back to outline), confirming first if a note or highlights would be lost with it. Card-level note and tags are entered **inline** below the article in a `note-editor` — an "Add notes & tags" block with a borderless textarea and an `input-tags` field — whose Save commits the note and clears the input; a hairline `section-rule` separates the card-level actions from this notes area once the two coexist. The saved note then drops into an **Existing notes** section just beneath, alongside every highlight: card note and highlights share one white container, `saved-item-card`, each with EDIT / REMOVE controls — one consistent "saved item" treatment for everything the reader has kept, shown right on the result without opening the flashcard. EDIT flips the saved item **itself** into an editor, in place — the note, its tag chips, and a CANCEL beside Save appear inside the `saved-item-card`, so a note under revision is never shown twice (the "Add notes & tags" block above only appears while the card has no saved note yet). Highlights edit the same way: EDIT (or activating the highlight in the text) turns that highlight's own item into the editor.

**Annotation.** `toolbar-selection` appears over any text selection: a light `surface` strip with hairline border and `shadow-sm` — matching the reference toolbar — offering two labeled actions, Highlight and Note (highlight + note). It is reached by both input paths: mouse selection surfaces it on release, and a touch **long-press** selection surfaces the same toolbar once the selection settles (a touch selection never fires a mouse event, so the app watches the selection itself, debounced past the handle-dragging); its buttons meet the 44px floor on touch. `dialog-note` is the near-square note dialog, and it appears at exactly one moment: when a note is **first created** from the toolbar's Note action — every later edit happens in place inside the highlight's saved item (see Saving). Its anatomy: a "Note" heading set in `heading` (so it always outranks the serif body text, which renders optically larger than sans at equal size) with ✕ close, a borderless textarea, then a separate **Tags** field (`input-tags`) below it, and a right-aligned DELETE · CANCEL · SAVE text-button row. In the tags field, typing a space commits the current text as a `chip-tag-removable` — a bordered monochrome pill with a trailing ✕ that removes it (Backspace on an empty field removes the last chip). Every tags field also ends in a ▾ button opening a **recent-tags dropdown** — the top 5 tags by use recency (recorded locally when a chip is committed, never synced), minus any already chips in that editor, with a newest-cards fallback before anything has been recorded — the PRD's guard against near-duplicate tag drift, one tap instead of retyping. Inside a dialog the menu opens upward so the dialog's own scroll never clips it. `highlight-mark` is the yellow wash on saved text, and the entire wash is an interactive target: hovering shows a pointer and a 2px ink underline beneath the highlight, and clicking (or tapping, or Enter when focused) anywhere on it opens that highlight's in-place editor in the list below, scrolled into view with the note focused — not just the marker. If a note is attached, a `note-marker` — the tiny numbered square box from the reference — sits superscript at the highlight's end as the visible indicator and an equivalent target.

**Device chrome.** `nav-top` is the persistent top bar. Its background and hairline span the screen, its contents share the centered content column, and its anatomy is fixed so nothing moves between pages: ← back, the Glossary wordmark (both on every page except Home), the Wikipedia search field (on Results, Collection, and card detail), then Home | Collection, the Settings gear, and the **account button** pinned to the column's right edge — those last four occupy the identical position on every page, Home included. The account button is the rightmost control: a person-outline `icon-btn` when signed out, the reader's Google avatar rendered **grayscale** when signed in (a color photo would be the only colored pixel in the app); it opens the login/account page. Home keeps its bar minimal (nav + gear + account only) because search there is the centered hero field. On narrow viewports, where the nav links live in the bottom bar instead, Settings and the account button stay pinned to the right edge of the top bar. The field keeps the full `input-search` anatomy everywhere (leading SVG icon, focus ring, trailing ✕ clear when it has input); the saved-flashcards search is *not* in the bar — it lives on the Collection page under the title (see Collection). On narrow viewports the wordmark is dropped from the bar (the bottom bar already carries Home) so the search field keeps a usable width beside back + gear. The gear is an `icon-btn`: a real button with the shared pill 2px-ink hover/focus outline. `nav-bottom` is the mobile 3.5rem two-item bar with underlined active state. `book-footer` is the signature element: a letterspaced-caps progress line in `text-muted` (CARD 3 OF 12 · 2 SAVED THIS WEEK) that gives flashcard browsing and review the cadence of a page footer. `slider-track`/`slider-thumb` build the brightness and warmth controls: a 2px track whose filled side is ink, a 1.5rem bordered thumb, − / + step buttons at the ends, and the current value above the thumb, as in the reference quick-settings sheet; brightness runs on the device's own 1–20 scale. `badge-drift` is the PRD's passive "updated on Wikipedia" indicator: a bordered caption pill in muted ink — noticeable when sought, silent otherwise. It appears wherever the frozen saved copy has diverged from live Wikipedia (results and detail). The saved copy never changes on its own; the detail view offers one reader-controlled `button-text` action, **UPDATE SAVED COPY**, that pulls in the live text and re-anchors existing highlights to it, warning via `toast` if any highlight text no longer matches. `toast` is the inverted-ink save confirmation, bottom-center, auto-dismissing — and it doubles as the honest failure signal: if a write can't be persisted (device storage full or blocked), the save is rolled back and the toast warns rather than falsely confirming.

**Source provenance & the dictionary.** For a **direct search** — pressing Enter on a typed term, or choosing the dictionary row in the suggestions — the app leads with the **dictionary definition**: checking what a word *means* is the reader's most frequent, lowest-intent job, so the definition is the first thing offered rather than a fallback. This path has its own reload-safe route (`#/define/<term>`) and falls through to a Wikipedia article only when the word has no entry (a multi-word phrase, or a proper noun like "Dan Vapid"), so a direct search never dead-ends. A Wikipedia result reached another way — a Wikipedia suggestion, a recent search, a synonym chip — still runs Wikipedia-first and keeps the dictionary as its *fallback*: a lookup Wikipedia can't answer with a single article — a genuine 404, or a low-value disambiguation stub (e.g. "ignominy", whose disambiguation lists only obscure works by that name) — resolves to a dictionary definition rather than dead-ending. The dictionary is a second *source*, and source is the ONLY thing that distinguishes the two kinds of result: highlighting, notes, tags, review, and export all treat a dictionary card identically to a Wikipedia one (drift is the sole exception — a dictionary entry has no Wikipedia revision to track, so its badge never appears). `badge-source` marks provenance where it aids a reading or saving decision: a bordered caption pill reading **WIKIPEDIA** or **DICTIONARY** on the Results header and card Detail. It is deliberately *absent* from the Collection overview grid, where it only added clutter; there, the card title carries the term and the source is a detail for when the card is opened. In the suggestions dropdown the definition leads: whenever the typed word has an entry, a compact `src-tag`-marked **dictionary row sits first**, above the Wikipedia suggestions (which stay unmarked — the app's whole identity is Wikipedia search, so unmarked reads as Wikipedia). A dictionary result adds three reading affordances the encyclopedic path never had: a `pronounce` line (IPA in reading serif plus a play control, shown only when the API actually returns audio — often it does not), the definition itself composed one line per part of speech, and a `synonyms` row of tappable tag chips that each launch a fresh lookup. On a disambiguation page — one reached via a Wikipedia suggestion or a recent search — the dictionary is offered as a `dict-option`, a full-width actionable row above the Wikipedia candidates, so a multi-topic term keeps its useful disambiguation while a plain word's definition is one tap away (a *direct* search for that word skips this — it already leads with the definition). When a disambiguation yields *no* usable candidates at all, the page never dead-ends on an empty heading: it falls through to the dictionary definition, or — failing that — the standard no-result screen with a way back. Definitions and phonetics come from the Free Dictionary API (dictionaryapi.dev); synonyms from Datamuse — both keyless and CORS-accessible from the browser, so the no-backend architecture holds. On the Collection card the term and its `star-btn` share one centered `card-head` row: the title flexes and wraps to a new line rather than running under the star, so even a long term like "Chimamanda Ngozi Adichie" never collides with the corner control.

**Account & export surfaces.** The login/account page (`#/login`) keeps the device voice through all three auth states: signed out it is a quiet pitch — a heading, a plain-language line on what signing in does, one `button-outline` **Continue with Google**, and text links to the Privacy and Terms pages (themselves plain reading-typography text pages, no chrome); signing in shows a progress state; signed in it shows the grayscale avatar large with name and email, a **Sign out** `button-text`, and **Delete account** as the standard quiet-destructive pattern — a `button-text` opening a confirm dialog that spells out what is purged and requires typing DELETE, never red. A first sign-in on a device that already holds local cards offers a one-time import dialog (the local deck is never merged silently — on a shared machine that would leak one person's cards into another's account); the same import lives in Settings' Sync group for later. The export sheet gains a second action when signed in: **EMAIL TO ME** beside Download, bundling the selection as one zip named `(DD-MM-YYYY) Glossary Export` — on a phone the file goes straight to the share sheet; on a computer it downloads while a Gmail compose tab opens pre-addressed to the account, and a caption on the sheet says exactly that, so the one manual attach step is never a surprise.
