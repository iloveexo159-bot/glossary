...
|---|---|---|---|
| **Search (Home) Page** | The default landing view every time the app opens. | Centered hero search input (Google-homepage style) under the wordmark, with live suggestions; recent-searches list directly below; top bar carries only nav access to Flashcards and Settings. | Get the reader typing immediately, and recover gracefully from a rushed or slightly mistyped search before it ever becomes a dead end. |
