...
- One-tap flashcard save, plus a highlight-to-save path: selecting text in a result surfaces a confirm action ("Highlight & save") rather than saving automatically, so plain text selection (e.g. to copy) never has a side effect. Saving checks the canonical Wikipedia title against existing flashcards first — if the term is already saved, it opens the existing card instead of creating a duplicate.
- Inline annotation popup on any highlight, with separate **note** and **tag** fields.
- Two tag levels — on a specific highlight, and on the whole flashcard — unified by a tag-filter that searches both, plus autocomplete against previously-used tags to stop near-duplicate tags (e.g. "Sapiens" vs. "sapiens") from fragmenting your organization.
- Two flashcard display modes (Overview and Review) sharing a single internal detail view; neither mode ever links to the live Wikipedia article. Review mode orders cards least-recently-reviewed first, so revision sessions surface neglected terms instead of the same easy ones each time.
...
- A quiet, passive "updated on Wikipedia" badge on a flashcard whose source article has changed since it was saved — no popup, no auto-replace; your saved content never changes on its own.
...
|---|---|---|---|
| **Search (Home) Page** | The default landing view every time the app opens. | Autofocused search input with live suggestions; recent-searches list; small nav access to Flashcards and Settings. | Get the reader typing immediately, and recover gracefully from a rushed or slightly mistyped search before it ever becomes a dead end. |
| **Search Results Page** | Shown after a search resolves. | Height-capped intro paragraph (selectable text); lead image; "via Wikipedia" credit line; Save button; disambiguation candidate list; no-result/offline message; "already saved" redirect state. | Present just enough to understand the term and either save it or leave; the confirm-to-highlight interaction and duplicate-save check both live here. |
| **Flashcards Page** | Grid/list of saved flashcards. | Overview/Review mode toggle; tag filter (spans both card- and highlight-level tags); multi-select (tap-driven select mode on touch, native shift/cmd-click on desktop); empty-state message for a new user. | Browse, revise, or select flashcards for export; Review mode surfaces least-recently-reviewed cards first. |
| **Flashcard Detail Page** | Full record for one flashcard. | Saved summary + image; highlighted spans with their notes/tags; card-level tags; edit/delete controls (hover-revealed on desktop, always-visible on touch). | Where annotation actually happens — adding, editing, or removing highlights, notes, and tags after the initial save. Text selection is precise mouse drag-select on desktop; touch adds a tap-to-select-sentence shortcut alongside manual drag, since dragging is inherently less precise on a touchscreen. |
| **Export Page (sheet/modal)** | Appears from Flashcards Page once cards are selected. | Selected-card summary; export-format confirmation; download trigger. | Bundle selected flashcards into Markdown (with embedded images) for Obsidian/Notion, understanding this hands off a file rather than syncing directly into either tool. |
...
flowchart TD
    ...
    Home -->|"Navigate"| Flashcards["Flashcards Page"]
    Flashcards -->|"Overview mode"| OverviewState["Card shows term + summary"]
    Flashcards -->|"Review mode"| ReviewState["Card shows term only, least-recently-reviewed first"]
    OverviewState -->|"Tap card"| Detail
    ReviewState -->|"Tap: flip"| ReviewFlipped["Card reveals summary"]
    ReviewFlipped -->|"Tap again"| Detail
    ...
    Flashcards -->|"Enter select mode"| MultiSelect["Multi-select cards"]
    MultiSelect -->|"Choose cards, tap Export"| ExportPage["Export Page"]
...
- **Annotate/enrich**: Either inline on Results (select text → confirm highlight, one extra tap), or later via Flashcards → Detail → select text → note/tag popup.
- **Revise/study**: Home → Flashcards (Review mode, least-recently-reviewed first) → flip through cards → occasionally into Detail. Never touches Search/Results.
- **Curate/export**: Flashcards (Overview mode) → multi-select → Export → download. Often a laptop-based session.
...
4. **Overview and Review modes.** Shared detail view; collapsed/flip state for Review; least-recently-reviewed-first ordering.
